...
| 自动化验证 | ✅ 已完成 | 后端 151 项测试通过，覆盖稳定原文、签章地址白名单、管理员进度脱敏、处方三方/病历两方验签、到期门禁、供应商超时/额度不足保护、人工复核锁、摘要不一致拒绝、私有存储、加密备份/恢复、可信问诊结束信令、医生资金隔离、财务临床隐私、生产开发回退封闭、安全总预检及后台登录限流 |
...
- [x] `CA_MOCK_SIGN`、图片章冒充签名及“未签先流转”路径全部不存在。
