...
> 注意：本文档只覆盖 CA 协议双录和结果查询；处方 PDF 签署、个人/企业签章和合同验签已另按放心签
> 标准 API 官方文档实现，详见接入说明。
>
> **当前状态（2026-07-24）：** 账号主体迁移和服务开通已完成；双录及三方 PDF 签署/验签代码已完成；
> 服务器密钥配置、真实人员双录和首份正式处方联调仍待执行。
> 后续操作和验收项统一在接入说明的“下一步操作清单”中打勾，本文以下内容保持供应商原始协议定义。
