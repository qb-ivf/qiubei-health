...
2. encrypted  = SM4_CBC_Encrypt(
                    ...
                    padding = ISO/IEC 7816-4,           # ⚠️ 0x80 后补 0x00；明文恰为整块时新增一个填充块
                    plaintext = bodyJson.encode("utf-8")                    #    不是 PKCS7！（已解密 SDK 密文实证）
                )
...
| V8 | SM4 整块边界填充 | 明文 `0123456789abcdef`（恰 16 字节） | `a9f4a4c761cf5110a2adbd8e1cdb79b025db47d12e40bf0803278053400e9f40`（32 字节密文 → 证明整块明文额外补一个 ISO7816-4 填充块） |
| V9 | SM4 双整块 | 明文 `0123456789abcdef0123456789abcdef`（32 字节） | `a9f4a4c761cf5110a2adbd8e1cdb79b0d0cdb4c80a112f917302699a23c67f4c74e73fb22408c9dbe2acab3ed307ecb8` |
...
## 三、Python 实现要点（S1 已落地，✅ 单测全绿）

实现：[backend/app/utils/sm_crypto.py](../backend/app/utils/sm_crypto.py)（纯函数，仅依赖 gmssl）+ [backend/app/services/tj_gateway.py](../backend/app/services/tj_gateway.py)（`tj_call`/`tj_upload_file`，httpx 异步）。

- [x] **填充是 ISO/IEC 7816-4，不是 PKCS7**——gmssl 的 `crypt_cbc` 自带 PKCS7 不可直接用（实测密文对不上），已手写 CBC 链（复用 `CryptSM4.one_round` 做单块加解密 + 手工 XOR/填充）。这是通过解密 SDK 密文 `49a9a1cb…`（明文尾部为 `0x80 0x00…`）实证的。
- [x] SM3 输出：`sm3_hash` 返回小写 hex，需 `.upper()` 与 SDK 的大写输出一致（V1/V4/V6）。
