...
>
> 当前基线：MVP（M1–M8）及监管代码均已落地；测试网关 9/9 通过，正式网关仍等待真实人员/药品补录后开启。

---

## 〇、进度快照（2026-07-26 更新）
...
| **S4 运营后台监控** | ✅ 完成 | 按接口统计/看报文/按日补采；连续失败与签到缺失告警已接入 |
| **S5 测试联调** | 🟡 待正式首批 | 测试冒烟 **9/9 msgCode=200**、反显及历史达标沿用均确认；正式密钥已发放并在本地安全暂存，余：T6 补录、生产预检/部署、首批核验 |
| **代码整体** | ✅ 完成 | 151 tests passed；含监管/CA/生产安全预检、Alembic、多实例信令与任务租约、静态资源自托管、知情同意门禁及后台登录限流 |
...
| M10 提审上线 | 🟡 进行中 | 两端正式 AppID 已配置；余真机回归、类目审核、版本提审及等保自查 |
...
设计取舍：
- **沿用 asyncio 后台任务**（`main.py` 的 sweep 模式）做 worker 与每日调度，Redis 分布式租约
  防止多实例重复消费；旧的 Celery/AES-MD5 占位骨架已删除，避免误调用错误协议。
...
- [x] 无药问诊闭环：医生必须保存主诉、现病史、ICD-10、诊断和医嘱后才能直接完成，记录为
  `audit_status=not_required`；禁止空处方，不进入药师审方、在线处方或处方核销上报。
  图文和视频均生成可追溯电子病历，并幂等生成该订单的挂号分成流水；图文按在线咨询采集，
  视频随终态订单采集在线复诊和电子病历。
- [x] 问诊完成态防改写：图文聊天仅在候诊/问诊中可发送，结束、审方、驳回、退款或取消后
  自动只读；最终“处方/电子病历”结果由后端在事务提交成功后推送，客户端不能伪造完成类型。

前端：

- [x] **患者端小程序**：医生详情页（视频问诊）新增"复诊声明"卡——声明勾选（不勾选不可支付）+ 首诊诊断填写 + 首诊材料拍照上传（≤4 张，支付成功后自动上传）；新增「问诊评价」页（星级 1–5 + 0–10 分滑条 + 内容/建议），订单列表完成/退款单显示"评价"入口，已评价只读回显；新增“电子病历档案”列表/详情，无药问诊不再跳空处方；图文问诊结束后聊天只读并进入对应处方或电子病历。
- [x] **医生端小程序**：开方页诊断改 **ICD-10 搜索多选**（chips，选中自动同步诊断文字，提交带 `icd_code/icd_name`）；药品搜索从内置演示库切换为**后端药品字典**（带 drug_id/限售拦截提示）；处方明细新增**用药天数**步进器；病历页展示患者实际留言/首诊材料并要求填写主诉和现病史；图文/视频均可选择“不开药，仅保存病历”，挂断视频不再直接生成无病历完成单。
...
- [x] 告警：正式上报启用后，同一接口连续失败达到
  `TJ_ALERT_FAILURE_THRESHOLD`（默认 3）或不良事件签到超过
  `TJ_ALERT_SIGNIN_DEADLINE_HOUR`（默认北京时间 03:00）未完成时，在监管 Dashboard
  固定展示告警；关闭正式上报时不产生误报。
