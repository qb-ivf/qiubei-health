...
- [x] 无药问诊闭环：医生可保存 ICD-10、诊断、现病史和医嘱后直接完成，记录为
  `audit_status=not_required`；禁止空处方，不进入药师审方、在线处方或处方核销上报。
  视频问诊仍随终态订单采集在线复诊和电子病历。

前端：

- [x] **患者端小程序**：医生详情页（视频问诊）新增"复诊声明"卡——声明勾选（不勾选不可支付）+ 首诊诊断填写 + 首诊材料拍照上传（≤4 张，支付成功后自动上传）；新增「问诊评价」页（星级 1–5 + 0–10 分滑条 + 内容/建议），订单列表完成/退款单显示"评价"入口，已评价只读回显；新增“电子病历档案”列表/详情，无药问诊不再跳空处方。
- [x] **医生端小程序**：开方页诊断改 **ICD-10 搜索多选**（chips，选中自动同步诊断文字，提交带 `icd_code/icd_name`）；药品搜索从内置演示库切换为**后端药品字典**（带 drug_id/限售拦截提示）；处方明细新增**用药天数**步进器；病历页顶部展示患者复诊声明 + 首诊材料图（可预览）；增加“不开药，仅保存病历”完成动作，挂断视频不再直接生成无病历完成单。
